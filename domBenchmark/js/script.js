...
function createDOMElements(count) {
    //first create scene, but *not* add to the document yet
    var s = document.createElement("scene");
    s.setAttribute("id", "testScene");

    //create the elements, add to scene
    ...
    //add the scene with all the elements to the document body at the end
    //document.body.appendChild(s);
}

function createJSElements(count) {
    purejs_ents = [];
    for (var i = 0; i < count; i++) {
...
var createElements = function() {
    ...
    var start = Date.now();
    if (do_dom) {
	createDOMElements(sliderValue);
    } else {
	createJSElements(sliderValue);
    }
